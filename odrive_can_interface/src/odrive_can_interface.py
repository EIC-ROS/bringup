...
def orientation(pos_left, pos_right):
    b = 0.333 # wheel base in m
    r = 0.085# wheel radius in m
    
    displacement_left = 2*math.pi*r * pos_left
    displacement_right = 2*math.pi*r * pos_right

    yaw = (displacement_right - displacement_left)/b
...
def read_can_data(bus):
    ...
    while not rospy.is_shutdown():
        ...
        if msg.arbitration_id == 0x09 | axisID_left << 5:
            ...
            pos_right = -msg['Pos_Estimate']
        V, w = instantaneous_vel(w_left, w_right)
        # yaw = orientation(pos_left, pos_right)
        
        # quarternion = tf.transformations.quaternion_from_euler(0, 0, yaw)

        Odom_msg.header.stamp = rospy.Time.now()
        Odom_msg.header.frame_id = "odom"
        Odom_msg.child_frame_id = "base_link"

        # Odom_msg.pose.pose.orientation.x = quarternion[0]
        # Odom_msg.pose.pose.orientation.y = quarternion[1]
        # Odom_msg.pose.pose.orientation.z = quarternion[2]
        # Odom_msg.pose.pose.orientation.w = quarternion[3]

        # Odom_msg.pose.covariance = [0.000000000001, 0, 0, 0, 0, 0,
        #                             0, 0.000000000001, 0, 0, 0, 0,
        #                             0, 0, 0.000000000001, 0, 0, 0,
        #                             0, 0, 0, 0.000000000001, 0, 0,
        #                             0, 0, 0, 0, 0.000000000001, 0,
        #                             0, 0, 0, 0, 0, 0.024858]

        Odom_msg.twist.twist.linear.x = V
        Odom_msg.twist.twist.angular.z = w

        Odom_msg.twist.covariance = [0.00003836904476, 0, 0, 0, 0, 0,
                                     0, 0.000000000001, 0, 0, 0, 0,
                                     0, 0, 0.000000000001, 0, 0, 0,
                                     0, 0, 0, 0.000000000001, 0, 0,
                                     0, 0, 0, 0, 0.000000000001, 0,
                                     0, 0, 0, 0, 0, 0.0000864813552]#0.00197813678
